...
def index():
    ...
    query = f"""
        SELECT * FROM binance_ohlcv_1h WHERE HOUR(open_time) = 9;        
    """
...
def calculate_score(coin_by_pair):
    ...
    for coin_pair in coin_by_pair:
        ...
        if  macd_line > macd_signal:
         ...
         # sma 조건
        sma_20 = yesterday["sma_20"]
        sma_50 = ta.sma(coin_pair["close_price"], length=50).iloc[-3]
        ...
        pair = yesterday["pair"] 
        score_lists_yesterday[pair] = score    
   
        coin_last_yesterday = latest_row.to_frame().T
        coin_last_yesterday["score"] = score
        
        coin_last_yesterday["score_text"] = coin_last_yesterday["score"].map(score_mapping)
        results_yesterday.append(coin_last_yesterday)
        
    return results, results_yesterday
...
if __name__ == "__main__":    
    df = index()
    coin_skills = fetch_count_rate(df)
