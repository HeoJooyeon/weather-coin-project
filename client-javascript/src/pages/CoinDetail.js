...
export async function renderCoinDetailPage(container, coinSymbol = "BTC") {
  ...
  try {
    // 1. 코인 리스트에서 해당 코인 정보 가져오기
    const coins = await fetchCoinsFromServerAPI();
    const coin = coins.find((c) => c.symbol === coinSymbol) || coins[0];

    if (!coin) {
      container.innerHTML = "<p>코인 정보를 찾을 수 없습니다.</p>";
      return;
    }

    // 2. pair 값으로 날씨/가격 데이터 가져오기
    const pair = coin.pair || `${coin.symbol}USDT`;
    const weatherData = await fetchCoinWeatherData(pair);

    // 3. 날씨 데이터 포맷팅 (formatWeatherData 사용)
    const forecasts = weatherData
      ? formatWeatherData(weatherData, coin.symbol)
      : [];
    ...
    // 가격 표시 (API 데이터 사용)
    const priceText =
      weatherData && weatherData.current_price
        ? `$${parseFloat(weatherData.current_price).toLocaleString()}`
        : "N/A";
    ...
    ["1일", "1주", "1개월", "1년", "전체"].forEach((time) => {
      const option = document.createElement("button");
      option.className =
        time === "1개월" ? "time-option selected" : "time-option";
    ...
    indicatorTypes.forEach((indType) => {
      ...
      tab.addEventListener("click", () => {
        ...
        if (
          ...
          typeof activeChartObjects["indicatorChartCanvas"].destroy ===
            "function"
        ) {
        ...
        if (
          ...
          console.log(
            `${indType} chart selected, but not implemented yet. Or btcPriceChartCanvas or its data is missing for MA.`,
          );
          // 다른 지표 차트 로직 (예: createRsiChart(indicatorChartCanvas, btcPriceChart._fullCandlestickDataForMA); )
        } else {
          console.warn(
            "BTC Price Chart is not available for indicator calculation.",
          );
        }
      });
    ...
    try {
      ...
      if (btcChart) {
        ...
        if (btcChart._fullCandlestickDataForMA) {
          ...
          console.warn(
            "Could not retrieve full OHLC data from BTC price chart for MA calculation.",
          );
        }
      }
    ...
    console.error("CoinDetail 페이지 렌더링 중 오류 발생:", error);
    container.innerHTML = `<p class='error-message'>데이터를 불러오는 데 실패했습니다: ${error.message}</p>`;
  }
}
